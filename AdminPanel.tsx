...
export const AdminPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'stores' | 'products' | 'orders' | 'reviews' | 'coupons' | 'subscriptions' | 'users' | 'mailbox'>('analytics');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 animate-fade-in text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-border-main pb-6 gap-4">
        ...
        <div className="flex space-x-1 bg-bg-secondary p-1 rounded-2xl border border-border-main self-start sm:self-auto overflow-x-auto scrollbar-none">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all flex items-center space-x-1.5 shrink-0 cursor-pointer relative ${
              activeTab === 'mailbox'
                ? 'bg-text-primary text-bg-primary shadow-sm'
                : 'text-text-secondary hover:text-text-primary'
            }`}
          >
            <FileText className="w-3.5 h-3.5" />
            <span>Nomination Mailbox</span>
            {users.some(u => u.adminRequest === 'pending') && (
      ...
      <AnimatePresence mode="wait">
        ...
        {/* Tab 8: User Directory Management */}
        {activeTab === 'users' && (
          <motion.div
            key="users-tab"
            initial={{ opacity: 0, y: 10 }}
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-left">
              <div>
                <h3 className="text-sm font-bold text-text-primary">Platform User Directory</h3>
                <p className="text-xs text-text-secondary mt-0.5 font-medium">Directly promote users to Shop Owners (Sellers) with Gold membership or manage roles.</p>
              </div>
              <div className="relative w-full sm:w-72">
                <input
                  type="text"
            ...
            <div className="bg-card-main border border-border-main rounded-3xl overflow-hidden shadow-sm">
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-left text-xs">
                  <thead>
                    <tr className="bg-bg-secondary border-b border-border-main text-[10px] font-bold uppercase tracking-wider text-text-tertiary">
                      <th className="p-4">User Details</th>
                      <th className="p-4">Platform Role</th>
                      <th className="p-4">Assigned Boutique</th>
                      <th className="p-4">Billing Sync</th>
                      <th className="p-4 text-right">Promotion Powers</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border-main text-text-primary">
                    {users
                      ...
                      .map((u) => {
                        const linkedStore = stores.find(s => s.ownerId === u.id || s.id === u.storeId);
                        return (
                          <tr key={u.id} className="hover:bg-bg-secondary/40 transition-all">
                            <td className="p-4">
                            ...
                            <td className="p-4">
                              {linkedStore ? (
                                <div className="flex items-center space-x-2">
                                  <img src={linkedStore.logo} alt="Logo" className="w-6 h-6 rounded-md object-cover border border-border-main bg-white" />
                                  <span className="font-semibold">{linkedStore.name}</span>
                                </div>
                              ) : (
                                <span className="text-text-tertiary">No Boutique Linked</span>
                              )}
                            </td>
                            <td className="p-4">
                              {u.role === 'seller' ? (
                                <span className="text-[10px] text-emerald-500 font-bold uppercase bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                                  {u.subscriptionTier || 'bronze'} ({u.subscriptionStatus || 'active'})
                                </span>
                              ) : (
                            ...
                            <td className="p-4 text-right">
                              {u.role === 'customer' ? (
                                <button
                                  onClick={async () => {
                                    if (confirm(`Direct Promotion: Upgrade "${u.name}" to a Shop Owner with Gold Tier membership for free?`)) {
                                      await promoteToSellerDirect(u.id);
                                      alert(`"${u.name}" is now a Shop Owner with Gold privileges!`);
                                    }
                                  }}
                                  className="px-3 py-1.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white text-[10px] font-bold transition-all shadow-sm cursor-pointer"
                                >
                                  Promote to Shop Owner (Free)
                                </button>
                              ) : u.role === 'seller' ? (
                                <span className="text-text-secondary text-[10px] italic">Already Shop Owner (Gold Tier)</span>
                              ) : (
                                <span className="text-red-500 text-[10px] font-bold">Platform Super Admin</span>
                              )}
                            </td>
                          </tr>
                        );
        ...
        {activeTab === 'mailbox' && (
          ...
          >
            {/* Header */}
            <div>
              <h3 className="text-sm font-bold text-text-primary">Super Admin Nomination Mailbox</h3>
              <p className="text-xs text-text-secondary mt-0.5 font-medium">Review administrative privilege requests submitted by shoppers or boutique owners.</p>
            </div>

            {/* Split layout */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
              {/* Left Column: Email list */}
              <div className="md:col-span-1 border border-border-main rounded-3xl bg-card-main overflow-hidden shadow-sm h-[400px] overflow-y-auto">
                <div className="bg-bg-secondary p-3 border-b border-border-main font-bold text-[10px] uppercase tracking-wider text-text-secondary">
                  Inbox Items ({users.filter(u => u.adminRequest === 'pending').length})
                </div>
                <div className="divide-y divide-border-main">
                  {users.filter(u => u.adminRequest === 'pending').map((u) => (
                    <div
                      key={u.id}
                      onClick={() => setSelectedEmailId(u.id)}
                      className={`p-4 cursor-pointer transition-all text-left space-y-1.5 ${
                        selectedEmailId === u.id 
                          ? 'bg-bg-secondary border-l-2 border-accent-main' 
                          : 'hover:bg-bg-secondary/40'
                      }`}
                    >
                      <div className="flex justify-between items-center text-[10px]">
                        <span className="font-bold text-text-primary">system-alerts@clothza.com</span>
                        <span className="text-text-tertiary">Just now</span>
                      </div>
                      <p className="font-semibold text-xs text-text-primary truncate">Admin Request: {u.name}</p>
                      <p className="text-[10px] text-text-secondary line-clamp-1">Nomination authorization to grant Super Admin privileges...</p>
                    </div>
                  ))}
                  
                  {users.filter(u => u.adminRequest === 'pending').length === 0 && (
                    <p className="text-xs text-text-secondary text-center py-12 px-4">Your administrative mailbox is empty. No pending nominations.</p>
                  )}
                </div>
              </div>

              {/* Right Column: Email details reader */}
              <div className="md:col-span-2 border border-border-main rounded-3xl bg-card-main shadow-sm p-6 h-[400px] flex flex-col justify-between">
                {selectedEmailId && users.find(u => u.id === selectedEmailId && u.adminRequest === 'pending') ? (() => {
                  const targetUser = users.find(u => u.id === selectedEmailId)!;
                  return (
                    <>
                      {/* Email Header */}
                      <div className="border-b border-border-main pb-4 space-y-3 text-left">
                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                          <h2 className="font-serif text-base font-bold text-text-primary">
                            [Nomination Link] Request for Administrative Privileges
                          </h2>
                          <span className="text-[10px] bg-bg-secondary text-text-tertiary border border-border-main px-2 py-0.5 rounded-full self-start sm:self-auto">
                            Action Required
                          </span>
                        </div>
                        <div className="space-y-1 text-[11px] text-text-secondary">
                          <p><strong>From:</strong> system-alerts@clothza.com</p>
                          <p><strong>To:</strong> main-admin@clothza.com</p>
                          <p><strong>Date:</strong> {new Date().toLocaleDateString([], { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                        </div>
                      </div>

                      {/* Email Body */}
                      <div className="flex-grow py-6 text-left space-y-4 text-xs text-text-secondary leading-relaxed overflow-y-auto">
                        <p>Hello Admin,</p>
                        <p>
                          A user has requested administrative role permissions on the **Clothza** marketplace console:
                        </p>
                        <div className="p-4 rounded-2xl bg-bg-secondary border border-border-main space-y-1">
                          <p><strong>Nominee Name:</strong> {targetUser.name}</p>
                          <p><strong>Nominee Email:</strong> {targetUser.email}</p>
                          <p><strong>Current Role:</strong> {targetUser.role}</p>
                        </div>
                        <p>
                          If you trust this individual and want to share the Super Admin Console dashboard, please click the secure approval link below. This will instantly promote their account permissions.
                        </p>
                      </div>

                      {/* Email Actions (Link Simulation) */}
                      <div className="border-t border-border-main pt-4 flex items-center justify-end space-x-3">
                        <button
                          onClick={async () => {
                            if (confirm(`Are you sure you want to reject the admin request from "${targetUser.name}"?`)) {
                              await approveAdminRequest(targetUser.id, false);
                              setSelectedEmailId(null);
                              alert("Nomination request has been rejected.");
                            }
                          }}
                          className="px-4 py-2 rounded-xl text-xs font-bold text-text-secondary hover:bg-bg-secondary transition-all cursor-pointer"
                        >
                          Deny Request
                        </button>
                        <button
                          onClick={async () => {
                            if (confirm(`CRITICAL SECURITY ACTION: Are you sure you want to elevate "${targetUser.name}" to Super Administrator? They will have full database moderation access.`)) {
                              await approveAdminRequest(targetUser.id, true);
                              setSelectedEmailId(null);
                              alert(`"${targetUser.name}" has been promoted to Super Administrator!`);
                            }
                          }}
                          className="px-5 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-xs font-bold transition-all shadow-sm cursor-pointer"
                        >
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center space-y-3">
                    <FileText className="w-12 h-12 text-text-tertiary opacity-40 animate-pulse" />
                    <div>
                      <h4 className="text-xs font-bold text-text-primary">No Message Selected</h4>
                      <p className="text-[10px] text-text-secondary mt-0.5">Select a nomination alert from the inbox on the left to read and review.</p>
                    </div>
                  </div>
                )}
              </div>
